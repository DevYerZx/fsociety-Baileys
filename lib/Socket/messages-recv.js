...
const makeMessagesRecvSocket = (config) => {
    ...
    const handleMessage = async (node) => {
        ...
        if (((_c = (_b = msg.message) === null || _b === void 0 ? void 0 : _b.protocolMessage) === null || _c === void 0 ? void 0 : _c.type) === WAProto_1.proto.Message.ProtocolMessage.Type.SHARE_PHONE_NUMBER && node.attrs.sender_pn) {
            ...
            await signalRepository.lidMapping?.storeLIDPNMappings([{ lid: node.attrs.from, pn: node.attrs.sender_pn }]);
            ev.emit('lid-mapping.update', { lid: node.attrs.from, pn: node.attrs.sender_pn });
        }
        try {
            await Promise.all([
                processingMutex.mutex(async () => {
                    ...
                    if (msg.messageStubType === WAProto_1.proto.WebMessageInfo.StubType.CIPHERTEXT) {
                        ...
                        if ((_b = msg.key.participant) === null || _b === void 0 ? void 0 : _b.endsWith('@lid')) {
                            // A LID is not a phone number. Never fall back to our own JID here:
                            // doing so makes messages from unknown LIDs look as if we sent them.
                            if (node.attrs.participant_pn) {
                                msg.key.senderPn = node.attrs.participant_pn;
                                msg.key.participantLid = msg.key.participant;
                                await signalRepository.lidMapping?.storeLIDPNMappings([{
                                    lid: msg.key.participant,
                                    pn: node.attrs.participant_pn
                                }]);
                                ev.emit('lid-mapping.update', { lid: msg.key.participant, pn: node.attrs.participant_pn });
                        ...
                        if (!(0, WABinary_1.isJidGroup)(msg.key.remoteJid) && (0, WABinary_1.isLidUser)(msg.key.remoteJid)) {
                            const phoneJid = node.attrs.sender_pn || node.attrs.peer_recipient_pn;
                            if (phoneJid) {
                                msg.key.senderLid = msg.key.remoteJid;
                                msg.key.senderPn = phoneJid;
                                await signalRepository.lidMapping?.storeLIDPNMappings([{
                                    lid: msg.key.remoteJid,
                                    pn: phoneJid
                                }]);
                                ev.emit('lid-mapping.update', { lid: msg.key.remoteJid, pn: phoneJid });
