...
const processMessage = async (message, { shouldProcessHistoryMsg, placeholderResendCache, ev, creds, keyStore, logger, options, getMessage, signalRepository }) => {
    ...
    const content = (0, messages_1.normalizeMessageContent)(message.message);
    // Keep unresolved LIDs intact. Their numeric user part is not a phone
    // number and must only be converted using a confirmed PN<->LID mapping.
        ...
        switch (protocolMsg.type) {
            case WAProto_1.proto.Message.ProtocolMessage.Type.HISTORY_SYNC_NOTIFICATION:
                ...
                if (process) {
                    ...
                    if (data.lidPnMappings?.length && signalRepository?.lidMapping) {
